...
class Heat_transfer_equation {
	...
	double x_step {1.0 / num_of_x_step}, time_step {1.0 / num_of_time_step};
...
Heat_transfer_equation::Heat_transfer_equation(
							...
							int _num_of_x_step, int _num_of_time_step):
							...
							num_of_x_step{_num_of_x_step}, num_of_time_step{_num_of_time_step}
{
}
...
void Heat_transfer_equation::approximation()
{
	...
	for (int i {0}; i < num_of_x_step + 1; ++i) {
		...
		cur_value2 = std::abs(equation(cur_x, 1));
		if (cur_value1 > max1) {
			max1 = cur_value1;
		}
		if (cur_value2 > max2) {
...
int main(int argc, char* argv[]) {
	...
	Heat_transfer_equation equation (u, left, right, begin, true, true, f, num_x, num_time);
