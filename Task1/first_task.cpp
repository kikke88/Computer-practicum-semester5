...
void L_U_print(const int size, const std::vector<std::vector<double>>& arr,
								std::ostream& stream, const std::vector<int>& indexes) {
	stream << "<<L>>" << std::endl;
	for (int i {0}; i < size; ++i) {
...
std::vector<int> main_func(const int size, std::vector<std::vector<double>>& arr, const std::vector<double>& b_arr,
	...
	for (int counter {0}; counter < size - 1; ++ counter) {
		...
		max_column(size, counter + 1, counter + 1, indexes, arr);
	}
